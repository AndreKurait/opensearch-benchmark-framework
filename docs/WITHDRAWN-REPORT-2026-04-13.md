> # ⚠️ WITHDRAWN — DO NOT CITE
>
> This report is retained for the record only. Its headline conclusion does not
> survive review. Specifically:
>
> * **Prices were hardcoded and wrong, including in ordering.** Real us-east-1
>   on-demand list has AMD Turin ~35% *more* expensive than Graviton4 (not
>   9-12%) and ~15% *more* expensive than Intel (not "6% cheaper"). The
>   price-performance conclusion inverts once corrected.
> * **The Intel parts are mislabelled.** m8i/c8i/r8i are Granite Rapids (Xeon 6),
>   not Emerald Rapids.
> * **n=1 with no repetitions.** The report's own EBS table implies run-to-run
>   variance near ±100%, larger than every effect it claims.
> * **The load generator ran on the system under test**, so a slower-per-core CPU
>   also slowed the measuring instrument.
> * **Concurrency was tied to instance family** (c=2, m=4, r=8 clients),
>   confounding it with heap and RAM, and holding the whole matrix far below
>   cluster saturation.
> * **The summary excluded the one configuration where Graviton4 won** (r-family
>   gp3-fast term throughput: r8g 3,875 vs r8a 3,152 ops/s) by filtering to
>   gp3-default.
> * **The committed manifests could not have produced these results** — they
>   targeted a Kubernetes service name the Helm chart never created.
>
